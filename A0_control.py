...
class Control:
    ...
    def __init__(self, reactor):
        self.input = self.construct_input(reactor)
        self.signal = {}
    ...
    def evaluate_signals(self, reactor, t):
        ...
        if 'powertable' in reactor.solve:
            ...
            # - psi_ytchen:
    #----------------------------------------------------------------------------------------------
    def construct_input(self,reactor):
        ...
        for line in lines:
            ...
            if len(word) > 0:
                ...
                # fuel
                    ...
                    if len(word)>6: # psi_ytchen: multilayer solid model
                        inp['fuel'].append( {'id':word[1], 'matid':word[2:5], 'rx':word[5:9], 'nrx':word[9:12], 'frx':word[12:15]} )
                        for i in range(3):
                            inp['fuel'][-1]['rx'][i] = float(inp['fuel'][-1]['rx'][i])
                            inp['fuel'][-1]['nrx'][i]= int(inp['fuel'][-1]['nrx'][i])
                            inp['fuel'][-1]['frx'][i]= float(inp['fuel'][-1]['frx'][i])
                    else:
                ...
                # wire-wrapped subassemblies
                    ...
                    inp['pipe'].append( {'id':word[1], 'type':'barerod', 'matid':word[2], 'dhyd':word[3], 'len':word[4], \
                                         'dir':word[5], 'areaz':word[6], 'nnodes':int(word[7]), 'signaltemp':'','p2d':word[8], 'sb2st':word[9]} )
