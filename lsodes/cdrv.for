...
!           error.. in nroc, nsfc, nnfc, or nnsc
   ...
   3     call  nsfc(n, r, ic, ia, ja, jlmax, isp(il), isp(jl), isp(ijl), jumax, isp(iu), isp(jutmp), isp(iju), isp(q), isp(ira), isp(jra), isp(irac), isp(irl), isp(jrl), isp(iru), isp(jru),  flag)
         if(flag .ne. 0)then
!           error.. in nroc, nsfc, nnfc, or nnsc
            return 
         end if
