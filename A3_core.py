        ...
        if 'fuelrod' in reactor.solve:
            ...
            for i in range(reactor.solid.nfuelrods):
                for iz in range(reactor.solid.fuelrod[i].nz):
                    if isinstance(reactor.solid.fuelrod[i].fuel[iz].type, list): # psi_ytchen: multilayer solid model
                        self.fuelvol += sum(reactor.solid.fuelrod[i].fuel[iz].vf)*reactor.solid.fuelrod[i].dz[iz]*reactor.control.input['fuelrod'][i]['mltpl'][iz]
                    else:
